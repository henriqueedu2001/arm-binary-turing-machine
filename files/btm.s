...
set_fsm:
	mov r0 #0xf000
	ldr r0, [r0]
	mov r1, #0x0501  @ carrega o high byte da FSM (i = 0)
	mov r2, #0x0005  @ carrega o low byte da FSM (i = 0)
	lsl r1, r1, #16  @ desloca a parte alta de r1
	orr r1, r1, r2   @ concatena high byte + low byte (i = 0)
	str r1, [r0]     @ store do estado da FSM (i = 0)
	add r0, r0, #4   @ atualiza a próxima posição de escrita
	mov r1, #0x0000  @ carrega o high byte da FSM (i = 1)
	mov r2, #0xff00  @ carrega o low byte da FSM (i = 1)
	lsl r1, r1, #16  @ desloca a parte alta de r1
	orr r1, r1, r2   @ concatena high byte + low byte (i = 1)
	str r1, [r0]     @ store do estado da FSM (i = 1)
	add r0, r0, #4   @ atualiza a próxima posição de escrita
	bx lr            @ retorno da subrotina
...
set_tape:
	...
	ldr r0, [r0]      @ carrega tape_addr em r0
	mov r1, #0x4144   @ high byte (i = 0)
	mov r2, #0x10ff   @ low byte (i = 0)
	lsl r1, r1, #16   @ shift do high byte para a parte alta de r1
	orr r1, r1, r2    @ concatenação high byte + low byte em r1
	str r1, [r0]      @ store do byte (i = 0)
	add r0, r0, #4    @ próxima posição de escrita
	mov r1, #0xffff   @ high byte (i = 1)
	mov r2, #0x1f51   @ low byte (i = 1)
	lsl r1, r1, #16   @ shift do high byte para a parte alta de r1
	orr r1, r1, r2    @ concatenação high byte + low byte em r1
	str r1, [r0]      @ store do byte (i = 1)
	add r0, r0, #4    @ próxima posição de escrita
	bx lr             @ retorno da subrotina
